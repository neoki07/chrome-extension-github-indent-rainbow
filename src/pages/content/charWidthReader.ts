import { applyFontInfo } from '@pages/content/fontInfo';
...
class DomCharWidthReader {
  ...
  constructor(requests: CharWidthRequest[]) {
  ...
  private _createDomElements(): void {
    ...
    applyFontInfo(regularDomNode);
    container.appendChild(regularDomNode);

    const boldDomNode = document.createElement('div');
    applyFontInfo(boldDomNode);
    boldDomNode.style.fontWeight = 'bold';
    container.appendChild(boldDomNode);

    const italicDomNode = document.createElement('div');
    applyFontInfo(italicDomNode);
...
export function readCharWidths(requests: CharWidthRequest[]): void {
  const reader = new DomCharWidthReader(requests);
  reader.read();
}
